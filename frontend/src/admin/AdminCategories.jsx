...
import { Plus, Trash2, Pencil, X, Grid, PackageOpen, Loader2, CheckCircle, Link as LinkIcon, Upload, Image } from 'lucide-react'
import { useAuth } from '../context/AuthContext.jsx'
import { API_BASE } from '../data/dishes.js'
import './AdminHome.css'

const emptyForm = { name: '', img: '', link: '/menu', order: 0 }
...
function ImageField({ label, value, onChange, token, rounded = false }) {
    ...
    <div className="form-row">
      ...
      {value && !uploading && (
        <div className="image-preview-wrap">
          <img
            src={value} alt="preview" className="image-preview"
            style={rounded ? { borderRadius: '50%', width: 80, height: 80, objectFit: 'cover' } : { borderRadius: 8, maxHeight: 100 }}
...
export default function AdminCategories() {
  ...
  async function refresh() {
    try { setLoading(true); const d = await apiCall('categories'); setItems(d.data) }
  ...
  const setImg = (setter) => (url) => setter(f => ({ ...f, img: url }))

  async function handleAdd(e) {
    e.preventDefault()
    setError(''); setSuccess('')
    if (!form.name || !form.img) { setError('Category name and image are required.'); return }
    setSubmitting(true)
    try {
      await apiCall('categories', 'POST', { ...form, order: Number(form.order) }, getToken())
  ...
  async function handleDelete(id, name) {
    ...
    try { await apiCall(`categories/${id}`, 'DELETE', null, getToken()); await refresh() }
    catch (err) { setError(err.message) }
  }

  function openEdit(item) {
    setEditItem(item)
    setEditForm({ name: item.name, img: item.img, link: item.link || '/menu', order: item.order || 0 })
    setEditErr('')
  }
  ...
  async function handleEditSave(e) {
    e.preventDefault()
    setEditErr('')
    if (!editForm.name || !editForm.img) { setEditErr('Name and image are required.'); return }
    setEditSub(true)
    try {
      await apiCall(`categories/${editItem._id}`, 'PUT', { ...editForm, order: Number(editForm.order) }, getToken())
      setEditItem(null)
  ...
  return (
    <div className="admin-home-page">
      <div className="dash-header">
        <div>
          <h1 className="dash-title">Manage Categories</h1>
          <p className="dash-sub">Add, edit, or remove food categories shown on the homepage.</p>
        </div>
      </div>
      ...
      <div className="home-page-layout">
        ...
        <div className="dash-card">
          <div className="card-head">
            <h3 className="card-title"><Plus size={16} /> Add New Category</h3>
          </div>
          <form className="add-home-form" onSubmit={handleAdd}>
            <div className="form-row">
              <label>Category Name</label>
              <input name="name" value={form.name} onChange={set(setForm)} placeholder="e.g. Biryani" />
            </div>
            <ImageField
              label="Category Image (round thumbnail)"
              value={form.img}
              onChange={setImg(setForm)}
              token={getToken()}
              rounded
            />
            <div className="form-row">
              <label><LinkIcon size={13} /> Click Link</label>
              <input name="link" value={form.link} onChange={set(setForm)} placeholder="/menu?cat=Starters" />
            </div>
            <div className="form-row">
            ...
            <button type="submit" className="add-home-submit" disabled={submitting}>
              {submitting ? <><Loader2 size={15} className="spin-icon" /> Adding…</> : <><Plus size={16} /> Add Category</>}
            </button>
          </form>
        </div>
        ...
        <div className="dash-card">
          <div className="card-head">
            <h3 className="card-title"><Grid size={16} /> All Categories</h3>
          ...
          {loading ? (
            ...
            <div className="empty-state"><PackageOpen size={36} /><p>No categories yet.</p><span>Add one using the form on the left.</span></div>
          ) : (
            <div className="home-table-wrap">
              <table className="home-table">
                <thead><tr><th>Image</th><th>Name</th><th>Link</th><th>Order</th><th>Actions</th></tr></thead>
                <tbody>
                  {items.map(item => (
                    <tr key={item._id}>
                      <td><img src={item.img} alt={item.name} className="row-thumb-round" /></td>
                      <td style={{ fontWeight: 600 }}>{item.name}</td>
                      <td style={{ color: '#6b7280', fontSize: '.8rem' }}>{item.link}</td>
                      <td>{item.order}</td>
                      <td>
        ...
        <div className="edit-modal-overlay" onClick={() => setEditItem(null)}>
          <div className="edit-modal" onClick={e => e.stopPropagation()}>
            <div className="edit-modal-head">
              <h3><Pencil size={15} style={{ verticalAlign: '-2px' }} /> Edit Category</h3>
            ...
            <form className="add-home-form" onSubmit={handleEditSave}>
              ...
              <div className="form-row">
                ...
                <input name="link" value={editForm.link} onChange={set(setEditForm)} />
              </div>
              <div className="form-row">
                <label>Sort Order</label>
                <input type="number" name="order" value={editForm.order} onChange={set(setEditForm)} min="0" />
              </div>
